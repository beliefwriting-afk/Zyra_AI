...
│       ├── icons.js        SVG 圖示（取代 emoji，跨平台一致）
...
**幾乎所有視覺調整都在 `src/css/tokens.css` 一個檔案裡。** 元件層一律用
`var(--…)`，不得寫死色值——這條守住了，換風格才會是改一個檔，而不是全域搜尋取代。

```
src/css/tokens.css      色彩、字級、圓角、陰影、字體　← 改這裡
src/css/layout.css      版面結構（側邊欄、頂欄、看板區）
src/css/components.css  元件外觀（按鈕、卡片、chip、面板）
src/js/icons.js         17 個 SVG 圖示，24×24、1.75 線寬、currentColor
src/img/                Logo（亮／深色兩版）與 favicon
```

### 三條不能破的規則

1. **不要改 HTML 的 class 名稱或 id** —— JS 靠它們抓元素，改了畫面會壞掉，而且不會報錯。
2. **深色模式要一起改。** `tokens.css` 有三個區塊：`:root`（亮色）、
   `@media (prefers-color-scheme:dark)`（跟隨系統）、`:root[data-theme="dark"]`（使用者明確選擇）。
   後兩者內容必須一致——只改一個，另一種情境就會壞。
3. **改完跑對比檢查**：
...
46 組前景／背景全部要達 WCAG AA。**這一步不能跳過**——柔和的配色最容易在這裡出事，
而「看起來還好」跟「看得清楚」是兩件事。實際做這版時第一次跑就有 13 組不合格。

### 一個踩過的坑

為了湊對比，很容易一路把淡底色推向白。數字會全過，但**標籤 chip 和選中狀態會變得看不出有底色**——
一塊看不出來的底色等於沒做。正確做法是**壓暗前景**，不是漂白背景。
